...
{
  bool lg = false;
  if(server.args() < 2) return server.requestAuthentication();
  ...
  for (uint8_t i = 0; i < 3; i++)
  {
    ...
    if (strcmp(us, username[i]) == 0)
    {
      ...
      {
        userid = i;
        Serial.printf("Jugador %d logeado", userid);
        lg = true;
        break;
      }
    }
