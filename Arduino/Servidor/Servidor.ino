...
{
  bool lg = false;
  if(server.args() < 2) server.send(400, "text/html", GameHTML);
...
void AddNewPlayer()
{
  if(strcmp(server.uri().c_str(), "/NewUser") == 0)
  ...
  server.send(200, "text/plain", "Todo ok");
}
...
void setup(void)
{
  ...
  // for(uint8_t i = 0; i < sizeof(enemiesSt); i++)
  //   enemiesSt[i] = 1;
  ...
  server.on("/NewUser", AddNewPlayer);
